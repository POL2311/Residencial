    ...
    async function load() {
        ...
        els.list.innerHTML = items.length ? items.map((item) => `
          <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4 flex items-start justify-between gap-3">
            <div>
              <div class="text-sm font-semibold text-slate-800">$${Number(item.monto || 0).toFixed(2)}</div>
              <div class="mt-1 text-xs text-slate-500">${escapeHtml(item.concepto || 'Sin concepto')} · ${escapeHtml(item.metodo || 'Sin metodo')}</div>
            </div>
            <div class="text-right">
              <div class="text-sm font-medium text-slate-700">${escapeHtml(item.fecha || '')}</div>
