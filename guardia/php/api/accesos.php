...
function get_residencial_id(PDO $pdo, int $uid): int {
  ...
  if ($rid<=0) out(false, ['error'=>'Tu usuario guardia no está asignado a un residencial.'], 403);
  return $rid;
}
function now_mx(): DateTime {
  return new DateTime('now'); // si tu server ya es MX, ok
}
...
/**
 * Evalúa si la visita permitiría acceso AHORITA
 * (sin registrar nada)
 */
function evaluar_visita(array $visita): array {
  ...
  if (($visita['estado'] ?? '') === 'cancelado') { $permitido=false; $motivo='Acceso cancelado.'; }
  if (($visita['estado'] ?? '') === 'vencido')   { $permitido=false; $motivo='Acceso vencido.'; }
  if (($visita['estado'] ?? '') === 'usado' && (int)($visita['uso_unico'] ?? 0) === 1) {
    $permitido=false; $motivo='Acceso de uso único ya utilizado.';
  }

  if ($permitido) {
    if ($today < ($visita['fecha_desde'] ?? '0000-00-00') || $today > ($visita['fecha_hasta'] ?? '9999-12-31')) {
      $permitido=false; $motivo='Fuera del rango de fechas permitido.';
    }
  }
  ...
  if ($permitido) {
    ...
    if (!time_in_range($hDesde, $hHasta, $now)) {
      $permitido=false; $motivo='Fuera del horario permitido.';
    }
  }
...
try {
  ...
  /**
   * GET action=buscar&code=XXXX
   * Regresa la visita + evaluación (sin registrar nada)
   */
  if ($method === 'GET' && $action === 'buscar') {
    ...
    if (!$visita) out(false, ['error'=>'Código no encontrado o no pertenece a este residencial.'], 404);

    // si ya venció y quedó pendiente, aquí NO lo cambiamos; eso lo hace el scan (o puedes hacerlo aquí si quieres)
  ...
  /**
   * POST action=scan
   * code, tipo_evento=entrada|salida|verificacion
   * override=permitido|denegado (opcional)  -> para botones manuales
  ...
  if ($method === 'POST' && $action === 'scan') {
    ...
    if (!in_array($tipoEvento, $allowedTipoEvento, true)) out(false, ['error'=>'tipo_evento inválido'], 422);

    $override = strv($_POST['override'] ?? '', 20); // permitido | denegado | ''
    if ($override !== '' && !in_array($override, ['permitido','denegado'], true)) out(false, ['error'=>'override inválido'], 422);
    ...
    // Si hay override, manda ese. Si no, usa evaluación real.
    ...
    // si permitido y uso único => marcar usado
  ...
  /**
   * GET action=hist&limit=20
   * Historial reciente del guardia (o del residencial)
   */
  if ($method === 'GET' && $action === 'hist') {
    $limit = max(1, min(100, intv($_GET['limit'] ?? 20, 20)));
