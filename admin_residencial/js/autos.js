    ...
    els.modalBody.innerHTML = `
      ...
      <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
        <div>
          ...
          <input name="placas" required maxlength="15" value="${escapeHtml(auto.placas || '')}" class="w-full rounded-2xl border border-slate-200 px-3 py-2 text-sm">
        </div>
        <div>
          <label class="mb-1 block text-xs text-slate-600">Tag ID</label>
          <input name="tag_id" maxlength="120" value="${escapeHtml(auto.tag_id || '')}" class="w-full rounded-2xl border border-slate-200 px-3 py-2 text-sm">
        </div>
        <div>
          <label class="mb-1 block text-xs text-slate-600">Modelo</label>
          <input name="modelo" maxlength="80" value="${escapeHtml(auto.modelo || '')}" class="w-full rounded-2xl border border-slate-200 px-3 py-2 text-sm">
        </div>
        <div>
          <label class="mb-1 block text-xs text-slate-600">Color</label>
          <input name="color" maxlength="40" value="${escapeHtml(auto.color || '')}" class="w-full rounded-2xl border border-slate-200 px-3 py-2 text-sm">
        </div>
      </div>
      ...
      <article class="rounded-[1.75rem] border border-slate-200 bg-white p-4 shadow-sm">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div class="flex gap-2">
            <button type="button" data-edit-auto="${escapeHtml(auto.auto_id)}" class="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm text-slate-700 hover:bg-slate-50">Editar</button>
          </div>
        </div>
      </article>
    `).join('');
